...
const zeitAPIClient = axios.create({
  ...
  headers: {
    Authorization: `Bearer ${actionConfig.zeitToken}`,
  },
  params: {
    teamId: actionConfig.teamId,
  },
})

// Run your GitHub Action!
Toolkit.run(async tools => {
  function fetchLastDeployment(params: any) {
    return zeitAPIClient
      .get('/v4/now/deployments', {
        params,
      })
  ...
  for (const strategy of strategies) {
    const deployment = await (strategy as any)()
  ...
  const { data: comments } = await tools.github.issues.listComments({
    ...tools.context.repo,
    issue_number: tools?.context?.payload?.pull_request?.number || -1,
  })
