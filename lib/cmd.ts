...
import { CmdContext, DeployOptions, CmdOptions, Deployment } from './types'

export interface LastCommentOptions
  extends Pick<CmdContext, 'githubApi' | 'githubContext'> {
...
const comment = (cmdContext: CmdContext) => async () => {
  ...
  const deploymentStrategies = [
    zeitApi.getDeployments({
      'meta-commit': githubContext.sha,
      ...(teamId && { teamId }),
    }),
    zeitApi.getDeployments({
      'meta-branch': githubContext.ref,
      ...(teamId && { teamId }),
    }),
  ...
  const lastDeployment = deployments.find(
    deployment => deployment?.url !== undefined,
  ) as Deployment
